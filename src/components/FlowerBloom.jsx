...
const createLeafGeometry = (size = 0.32) => {
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    positions.setZ(
      i,
      z + t * t * size * 0.18 + (1 - Math.abs(x) / (w * 1.1)) * t * 0.012
    );
  }
  geometry.computeVertexNormals();
  return geometry;
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.scale.setScalar(
        scale * (1 + Math.sin(time * 1.1 + index * 0.7) * 0.008)
      );
    }
  });
...
const PetalLayer = ({
  ...
  const { baseRadius, radiusGrow, tiltBase, tiltDecay, randomPos, randomRot } =
    profile.arrange;
  ...
  const colors = useMemo(() => {
    ...
    return {
      ...
      light:
        "#" +
        new THREE.Color()
          .setHSL(hsl.h, hsl.s * 0.9, Math.min(hsl.l * 1.1, 0.92))
          .getHexString(),
    };
  }, [color]);

  const geometry = useMemo(
    () => createPetalGeometry(petalType, actualWidth, actualLength),
    [petalType, actualWidth, actualLength]
  );
  ...
  useFrame((state) => {
    if (groupRef.current) {
      ...
      groupRef.current.rotation[axis] =
        state.clock.getElapsedTime() * 0.035 + layerIndex * 0.12;
    }
  });
...
const FlowerCenter = ({ config, isSSR }) => {
  ...
  useFrame((state) => {
    if (groupRef.current)
      groupRef.current.scale.setScalar(
        1 + Math.sin(state.clock.getElapsedTime() * 1.6) * 0.018
      );
  });
  ...
  const seeds = useMemo(() => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      items.push({
        p: [Math.cos(angle) * r, 0.022, Math.sin(angle) * r],
        s: 0.01 + Math.random() * 0.004,
      });
    }
    return items;
  }, [size]);
  ...
  const dots = useMemo(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      items.push([
        Math.cos(theta) * Math.sin(phi) * size * 0.88,
        Math.sin(theta) * Math.sin(phi) * size * 0.88,
  ...
  if (special === "sunflower") {
    return (
      <group ref={groupRef} position={pos} rotation={rot}>
        <mesh>
          <cylinderGeometry args={[size, size * 0.88, 0.045, 28]} />
          <meshStandardMaterial color="#5D4537" roughness={0.88} />
        </mesh>
        {seeds.map((seed, i) => (
          <mesh key={i} position={seed.p}>
            <sphereGeometry args={[seed.s, 5, 5]} />
            <meshStandardMaterial
              color={i % 3 === 0 ? "#2D1F0F" : "#4A3020"}
              roughness={0.78}
            />
          </mesh>
        ))}
    ...
    return (
      <group ref={groupRef} position={pos} rotation={rot}>
        <mesh>
          ...
          <meshStandardMaterial color="#90B080" roughness={0.52} />
        </mesh>
        {[...Array(6)].map((_, i) => (
          <group key={i} rotation={[0.3, (i / 6) * Math.PI * 2, 0]}>
            <mesh position={[0, 0.1, 0]}>
              <cylinderGeometry args={[0.005, 0.005, 0.16, 5]} />
              <meshStandardMaterial color="#7CB040" roughness={0.48} />
            </mesh>
            <mesh position={[0, 0.19, 0]}>
              <capsuleGeometry args={[0.016, 0.028, 4, 5]} />
              <meshStandardMaterial color="#8B5020" roughness={0.55} />
            </mesh>
          </group>
        ))}
      </group>
  ...
  if (special === "lotus") {
    return (
      <group ref={groupRef} position={pos} rotation={rot}>
        <mesh>
          <cylinderGeometry args={[size * 0.48, size * 0.35, 0.09, 10]} />
          <meshStandardMaterial color="#DAA520" roughness={0.58} />
        </mesh>
        {[...Array(6)].map((_, i) => {
          ...
          return (
            <mesh
              ...
              position={[
                ...
                0.05,
                Math.sin(angle) * size * 0.26,
              ]}
            >
              <sphereGeometry args={[0.016, 5, 5]} />
              <meshStandardMaterial color="#556B2F" roughness={0.65} />
            </mesh>
          );
        })}
      </group>
    ...
    <group ref={groupRef} position={pos} rotation={rot}>
      <mesh>
        ...
        <meshStandardMaterial
          color={centerColor}
          roughness={0.45}
          metalness={isSSR ? 0.1 : 0.02}
        />
      </mesh>
      {dots.map((dot, i) => (
        <mesh key={i} position={dot}>
          <sphereGeometry args={[size * 0.07, 4, 4]} />
          <meshStandardMaterial
            color="#FFD700"
            emissive="#FFD700"
      ...
      items.push(
        faceForward
          ? {
              ...
              rotation: [0.62, 0, angle + Math.PI / 2],
            }
          : {
              position: [
                Math.cos(angle) * 0.045,
...
const SSRGlow = ({ color, glowColor, faceForward }) => {
  ...
  return (
    <mesh
      ref={ref}
      position={faceForward ? [0, 0, -0.18] : [0, -0.09, 0]}
      rotation={faceForward ? [0, 0, 0] : [Math.PI / 2, 0, 0]}
    >
...
const HydrangeaCluster = ({ color }) => {
  ...
  useFrame((state) => {
    if (ref.current)
      ref.current.rotation.y = state.clock.getElapsedTime() * 0.055;
  });
  ...
  const flowers = useMemo(() => {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      items.push([
        Math.cos(theta) * Math.sin(phi) * 0.32,
        Math.sin(theta) * Math.sin(phi) * 0.32,
        Math.cos(phi) * 0.32,
      ]);
    }
    return items;
  ...
  const petalGeom = useMemo(
    () => createPetalGeometry("hydrangea", 0.05, 0.058),
    []
  );
  ...
  const colors = useMemo(() => {
    ...
    return [
      color,
      "#" +
        new THREE.Color().setHSL(hsl.h + 0.035, hsl.s, hsl.l).getHexString(),
      "#" +
        new THREE.Color()
          .setHSL(hsl.h - 0.035, hsl.s * 0.92, hsl.l * 1.06)
          .getHexString(),
    ];
  }, [color]);
        ...
        <group key={fi} position={pos}>
          {[0, 1, 2, 3].map((pi) => {
            ...
            return (
              <mesh
                key={pi}
                position={[Math.cos(angle) * 0.014, Math.sin(angle) * 0.014, 0]}
                rotation={[0.12, 0, angle + Math.PI / 2]}
                geometry={petalGeom}
              >
                <meshStandardMaterial
                  color={colors[fi % 3]}
  ...
  useFrame((state) => {
    if (ref.current)
      ref.current.rotation.z =
        Math.sin(state.clock.getElapsedTime() * 0.45) * 0.035;
  });
        ...
        <group key={i} position={[0, f.y, 0]} scale={f.scale}>
          {[0, 1, 2, 3, 4, 5].map((p) => {
            ...
            return (
              <mesh
                key={p}
                position={[Math.cos(angle) * 0.022, 0, Math.sin(angle) * 0.022]}
...
const Stem = ({ height = 1.45, curve = 0.18 }) => {
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i <= 20; i++) {
      ...
      points.push(
        new THREE.Vector3(
          Math.sin(t * Math.PI * 1.05) * curve * (1 - t * 0.32),
          -height * t,
          Math.cos(t * Math.PI * 0.65) * curve * 0.22 * (1 - t * 0.45)
        )
      );
    }
  ...
  return (
    <mesh geometry={geometry}>
      <meshStandardMaterial color="#2A5520" roughness={0.62} />
    </mesh>
  );
};
...
const Leaf = ({ position, rotation, size = 0.28, delay = 0 }) => {
  ...
  useFrame((state) => {
    if (ref.current)
      ref.current.rotation.z =
        rotation[2] +
        Math.sin(state.clock.getElapsedTime() * 0.65 + delay) * 0.045;
  });
  return (
...
const StemLeaves = ({ curve = 0.18 }) => (
  <group>
    <Leaf
      position={[curve * 0.45 + 0.065, -0.32, 0.025]}
      rotation={[0.32, -0.22, 0.52]}
      size={0.28}
      delay={0}
    />
    <Leaf
      position={[-curve * 0.22 - 0.085, -0.58, -0.035]}
      rotation={[0.3, 0.32, -0.55]}
      size={0.32}
      delay={0.45}
    />
    <Leaf
      position={[curve * 0.12 + 0.045, -0.85, 0.018]}
      rotation={[0.26, -0.18, 0.42]}
      size={0.26}
      delay={0.9}
    />
  </group>
...
const BaseLeaves = () => {
  const leaves = useMemo(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      items.push({
        position: [Math.cos(angle) * 0.1, -1.38, Math.sin(angle) * 0.1],
        rotation: [
          0.72 + Math.random() * 0.22,
          angle + (Math.random() - 0.5) * 0.28,
          0,
        ],
        size: 0.28 + Math.random() * 0.08,
      });
    }
    return items;
  }, []);
    ...
    clone.traverse((child) => {
      if (child.isMesh && child.geometry) {
        ...
        const shouldKeep = (v0, v1, v2) => {
          if (clipDirection === ">") {
            return (
              v0 > clipThreshold && v1 > clipThreshold && v2 > clipThreshold
            );
          }
        ...
        if (indices) {
          ...
          for (let i = 0; i < indices.length; i += 3) {
            ...
            const v1 =
              clipAxis === "z"
                ? pos.getZ(indices[i + 1])
                : clipAxis === "y"
                ? pos.getY(indices[i + 1])
                : pos.getX(indices[i + 1]);
            const v2 =
              clipAxis === "z"
                ? pos.getZ(indices[i + 2])
                : clipAxis === "y"
                ? pos.getY(indices[i + 2])
                : pos.getX(indices[i + 2]);
          ...
          if (newNormal)
            geo.setAttribute(
              "normal",
              new THREE.Float32BufferAttribute(newNormal, 3)
            );
        }
...
const CompleteFlower = ({ flower, config }) => {
    ...
    return (
      <group>
        ...
        <group position={[0, 0.35, 0]}>
          <Stem height={1.28} curve={0.14} />
          <StemLeaves curve={0.14} />
        </group>
        <BaseLeaves />
      </group>
    );
    ...
    return (
      <group>
        ...
        <group position={[0, 0.24, 0]}>
          <Stem height={1.18} curve={0.1} />
          <StemLeaves curve={0.1} />
        </group>
        <BaseLeaves />
      </group>
    );
  ...
  return (
    <group>
      <group position={[0, 0.4, 0]}>
        ...
        {[...Array(layers)].map((_, layerIndex) => (
          <PetalLayer
            ...
            config={config}
            color={
              isSSR && flower.gradientColors
                ? flower.gradientColors[
                    layerIndex % flower.gradientColors.length
                  ]
                : flower.color
            }
      ...
      <group position={[0, 0.4, 0]}>
        <Stem height={1.48} curve={curveStrength} />
        <StemLeaves curve={curveStrength} />
      </group>
      <BaseLeaves />
    </group>
  );
